      ...
      <div className="w-full max-w-screen-2xl mx-auto px-6 lg:px-12">
        <div className="grid grid-cols-1 md:grid-cols-4 gap-12 mb-16">
          ...
          <div className="col-span-1 md:col-span-1">
            ...
            <p className="text-sm font-sans tracking-wide text-muted-foreground mb-6 max-w-xs leading-relaxed">
              {dict.about?.story?.slice(0, 120) || (isAr ? 'مجوهرات فضية فاخرة عيار 925 عريقة ومصممة بأعلى مستويات الأناقة.' : 'Mastery in 925 Silver. A legacy of elegance, designed for the modern connoisseur.')}...
            </p>
          </div>

          <div>
            <h4 className="text-[10px] tracking-[0.2em] uppercase text-primary font-medium mb-6">
          ...
          <div>
            <h4 className="text-[10px] tracking-[0.2em] uppercase text-primary font-medium mb-6">
              {dict.footer?.social_media || (isAr ? 'روابط سريعة' : 'Social')}
            </h4>
            <ul className="space-y-4">
        ...
        <div className="pt-8 border-t border-primary/10 flex flex-col md:flex-row justify-between items-center gap-4">
          <p className="text-xs text-muted-foreground tracking-wide">
            {dict.footer?.rights || `© ${new Date().getFullYear()} Baher Silver. All rights reserved.`}
          </p>
          <div className="flex gap-4 text-xs text-muted-foreground">
            <span>925 Sterling Silver Guaranteed</span>
          </div>
        </div>
      </div>
    </footer>
