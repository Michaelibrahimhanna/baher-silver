...
export function InstagramGallery({ dict, locale }: { dict?: Dictionary; locale?: string }) {
  const isAr = locale === 'ar';
  return (
    <section className="py-24 bg-background relative overflow-hidden">
      <div className="w-full flex flex-col items-center text-center mb-12">
        <h2 className="text-2xl font-serif text-primary mb-2 tracking-widest">
          {dict?.instagram?.handle || '@bahersilver'}
        </h2>
        <span className="text-[10px] tracking-[0.3em] uppercase text-muted-foreground font-semibold">
          {dict?.instagram?.join_community || (isAr ? 'انضم إلى مجتمعنا على إنستغرام' : 'Join our community on Instagram')}
        </span>
      </div>

      <div className="grid grid-cols-2 md:grid-cols-4 w-full border-t border-b border-white/5">
        {IMAGES.map((src, idx) => (
          <motion.div 
            key={idx}
